...
\end{tikzpicture}
\caption{Arquitectura NN para nuestro auto-encoder.}
\label{fig:nn}
\end{figure}

Observamos en la figura \ref{fig:nn} que la cantidad de inputs/outputs que tenemos es de $784$ ya que, como mencionamos anteriormente, las imágenes del MNIST son de $28x28$. Un dato para agregar respecto a la arquitectura que vamos a usar es que vamos a introducir una no-linearidad $ReLU$ para la capa de {\bf input} y la {\bf hidden layer}. Por otro lado, para la capa de {\bf output} vamos a introducir la función de activación $Sigmoide$, ya que sabemos que las imágenes del MNIST están en escala de grises y los valores de sus pixeles se limitan al rango $(0, 1)$.
